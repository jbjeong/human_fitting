...
#                b_body_pose[:,48:51] -= 0.40
        ...
        #        sphere.translate(body_vs[7000])
        #        sphere.paint_uniform_color([1, 0, 0.7])
        #        vis_list.append(sphere)
        #        sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.03)
        #        sphere.translate(moved_body_vs[7000])
        #        sphere.paint_uniform_color([1, 0, 0.7])
        #        vis_list.append(sphere)
        #
        #        body_mesh.paint_uniform_color([1, 0.7, 0])
        #        moved_body_mesh.paint_uniform_color([0, 1, 0.7])
                #o3d.visualization.draw_geometries([orig_mesh, body_mesh, moved_body_mesh])
                vis_list = []
        
                #import pdb; pdb.set_trace()
                #for vidx in np.where(model.J_regressor[14] > 0)[0]:
                vidxs = np.arange(6065, 6070)
                for vidx in vidxs:
                    sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.002)
                    sphere.translate(body_vs[vidx])
                    sphere.paint_uniform_color([1, 0, 0])
                    vis_list.append(sphere)
        #        sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.001)
        #        sphere.translate(orig_vs[2161])
        #        sphere.paint_uniform_color([0.7, 0, 1])
        #        vis_list.append(sphere)
        #        sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.01)
        #        sphere.translate(body_vs[6115])
        #        sphere.paint_uniform_color([0, 1, 0.7])
        #        vis_list.append(sphere)
        #        o3d.visualization.draw_geometries([orig_mesh, body_mesh] + vis_list)
        #        o3d.visualization.draw_geometries([body_mesh] + vis_list)
        ...
        #            vert2 = moved_body_vs[vidx]
        #            dist2 = np.sqrt(((vert2 - moved_body_vs)**2).sum(1))
        #            v2s = moved_body_vs[dist2.argsort()[:K]]
...
#                for tri_idx in range(orig_triangles.shape[0]):
#                    vidx1, vidx2, vidx3 = orig_triangles[tri_idx]
#                    x1 = moved_rp_verts[vidx1]
#                    x2 = moved_rp_verts[vidx2]
#                    x3 = moved_rp_verts[vidx3]
#
#                    area = get_triangle_area(x1, x2, x3)
#                    if area > 0.003:
#                        dist1to2 = np.sqrt(((x1 - x2)**2).sum())
#                        dist1to3 = np.sqrt(((x1 - x3)**2).sum())
#                        dist2to3 = np.sqrt(((x2 - x2)**2).sum())
#
#                        target_idx = None
#                        target = None
#                        a_idx = None
...
#                                b_idx = vidx3
#
#                        R1 = Rs[orig_to_body_list[a_idx]]
#                        t1 = ts[orig_to_body_list[a_idx]]
#                        R2 = Rs[orig_to_body_list[b_idx]]
#                        t2 = ts[orig_to_body_list[b_idx]]
#
#                        new_vert1 = R1 @ target + t1
#                        new_vert2 = R2 @ target + t2
#
#                        new_vert = (new_vert1 + new_vert2) / 2
#
#                        moved_rp_verts[target_idx] = new_vert
